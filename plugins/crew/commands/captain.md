...
3. Implement the plan yourself, step by step, with a todo list. You are the
...
4. Dispatch reviewers **in parallel** on the changed files: **carpenter**
   ...
   if the change touches pupil data, permissions, auth, `settings.py` or an
   external API.
5. Fix every High finding. Fix Medium findings unless there is a reason not to,
   and state the reason.
6. Dispatch **gunner** to write tests for what you built. Gunner runs them and
   reports the real result.
7. Dispatch **lookout** to walk the finished workflow end to end.
...
4. If the fix touched pupil data, permissions or auth, dispatch
   **master-at-arms** before you call it done.
...
Run `/wheels-up` and follow it. Do not shortcut it.

---

## 3. The standing order that overrides the passage

**If the task touches the `tolerance`, `sdq`, `flashcards` or `evaluation` apps,
or any model holding pupil-linked data, the Gauntlet applies** — no matter which
passage you chose, including Fix and Tidy. Load the `gauntlet` skill and follow
it before you report the work complete. It is not optional and it is not
something the user has to remember to ask for.
...
- **Dispatch independent reviewers in parallel** — multiple agent calls in one
  ...
  Gunner and Lookout are serial because they write files and need the final
  state.
