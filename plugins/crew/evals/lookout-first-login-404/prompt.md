---
description: settings.py has no LOGIN_REDIRECT_URL, so a teacher who logs in from the login page is sent to /accounts/profile/ and gets a 404. Every view works; only the walk from the login page finds it. Asked through /crew:captain, whose Look passage sends whole-workflow QA to the Lookout. The Lookout must walk the flow with the test client, report the 404, and leave the tree untouched.
