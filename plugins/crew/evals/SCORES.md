...
| 2.15.0 | 2026-09-21 | sonnet | | | | | | 1.00 / 1.00 | | 1.00 / 1.00 | | | | | Issues #9 and #10, candidate 1 of each. New case surgeon-third-bad-day: the dashboard rates a pupil high on a third dysregulated observation dated exactly 28 days ago, inside a "four week" window that is 29 days; data from a new seed_demo command, scaffolded with seed.sh --demo. Gunner case gains a second hole, an unfiltered ?pupil= on the list, with a weight-2 regex grader. Both reproduced by hand and in fixture_check. No agent changed. WSL2, 1 run per arm. Surgeon: crew 1.39, baseline 0.48; Surgeon dispatched, Bash 6x in the crew arm and 2x in the baseline, so plain Claude built a loop too and ruled out an alternative on two votes of three. Gunner: crew 1.77, baseline 0.45; both arms wrote and ran the ?pupil= test. Two draws: on both, plain Claude now does the method the agent was written to enforce. Bash 6x with the Surgeon dispatched is the first evidence that tool_used sees a subagent's calls, unless the Captain ran them itself; the --keep-temp check in docs/handoff.md still decides it. |
